...
StatusOr<std::string> ShellySwitch::GetInfoJSON() const {
  ...
  std::string res = mgos::JSONPrintStringf(
      ...
      "lb_timer_rate: %d, lb_timer_step: %d, lb_timer_always_on_enabled: %B, lb_timer_start_value: %d ",
      ...
      cfg_->lb_timer_rate, cfg_->lb_timer_step, cfg_->lb_timer_always_on_enabled, cfg_->lb_timer_start_value);
  if (out_pm_ != nullptr) {
...
Status ShellySwitch::SetConfig(const std::string &config_json,
  ...
  json_scanf(
      ...
      &cfg.lb_timer_rate, &cfg.lb_timer_step, &cfg.lb_timer_always_on_enabled, &cfg.lb_timer_start_value);
  ...
  if (cfg.svc_type == 4) {
    ...
    if (cfg.lb_timer_start_value < 0) {
      cfg.lb_timer_start_value = 0;
    } else if (cfg.lb_timer_start_value > 100) {
      cfg.lb_timer_start_value = 100;
    }

    if (cfg_->lb_timer_rate != cfg.lb_timer_rate || cfg_->lb_timer_always_on_enabled != cfg.lb_timer_always_on_enabled) {
      cfg_->lb_timer_rate = cfg.lb_timer_rate;
      cfg_->lb_timer_always_on_enabled = cfg.lb_timer_always_on_enabled;
      ArmUpdateTimer();
    }
    if (cfg_->lb_timer_step != cfg.lb_timer_step) {
      cfg_->lb_timer_step = cfg.lb_timer_step;
      *restart_required = true;
    }
    if (cfg_->lb_timer_start_value != cfg.lb_timer_start_value) {
      cfg_->lb_timer_start_value = cfg.lb_timer_start_value;
    }
  } else {
...
void ShellySwitch::AutoOffTimerCB() {
  ...
  if (!cfg_->auto_off && cfg_->svc_type != 4) return;
  ...
  LOG(LL_INFO, ("Auto off timer expired, turning off"));
...
void ShellySwitch::SendNotifications() {
  LOG(LL_INFO, ("Sending characteristic notifications"));
